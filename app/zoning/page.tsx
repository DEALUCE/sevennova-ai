...
function Row({ k, v }: { k: string; v: ReactNode }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: 14 }}>
      ...
      <span style={{ color: '#e2e8f0', fontWeight: 500, textAlign: 'right', maxWidth: '58%' }}>{v ?? '—'}</span>
    </div>
  )
}
  ...
  return (
    <div style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '22px' }}>
      <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#64748b', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: accent || '#f5a623', display: 'inline-block' }} />
        {label}
      </div>
      {children}
    </div>
    ...
    <div style={s}>
      ...
      <div style={{ maxWidth: 700, margin: '80px auto 0', padding: '0 24px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#64748b', fontSize: '1.05rem', lineHeight: 1.6, marginBottom: 44 }}>
          Enter any Los Angeles address. Get APN, zoning, assessed value, TOC tier, SB 9 eligibility, ADU potential, development upside, and every grant you qualify for — instantly.
        </p>

        {/* Search */}
        <div style={{ display: 'flex', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, overflow: 'hidden', maxWidth: 600, margin: '0 auto 16px' }}>
          <input
            value={address}
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '60px 24px', color: '#64748b' }}>
          ...
          <p>Pulling APN, zoning, SB 9, ADU, and grant data…</p>
      ...
      {data && (
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px 80px' }}>
          ...
          <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: 24, marginBottom: 32 }}>
            <div style={{ fontSize: '1.4rem', fontWeight: 700, letterSpacing: '-0.01em', marginBottom: 6 }}>{data.full_address || data.address}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            ...
            <Card label="Parcel Data">
              ...
              <Row k="Lot Size" v={data.parcel?.sqft_lot ? `${data.parcel.sqft_lot.toLocaleString()} sq ft (${data.parcel.lot_acres} ac)` : '—'} />
              <Row k="Dimensions" v={data.parcel?.lot_dimensions} />
              <Row k="Beds / Baths" v={data.parcel?.bedrooms && data.parcel?.bathrooms ? `${data.parcel.bedrooms} bd / ${data.parcel.bathrooms} ba` : '—'} />
              <Row k="Corner Lot" v={pill(data.parcel?.lot_corner, 'Yes', 'No')} />
            ...
            <Card label="Zoning & Overlays">
              ...
              <div style={{ margin: '14px 0 8px', borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: 12, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#64748b' }}>Transit Overlays</div>
              <Row k="TOC Tier" v={data.overlays?.toc_tier > 0
                ? <span style={{ background: 'rgba(76,175,125,0.15)', color: '#4caf7d', padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>Tier {data.overlays.toc_tier}</span>
                : <span style={{ background: 'rgba(232,197,74,0.15)', color: '#e8c54a', padding: '3px 10px', borderRadius: 20, fontSize: 12 }}>None</span>} />
              {data.overlays?.toc_eligible && (
                <>
                  <Row k="Metro Station" v={data.overlays.nearest_metro_station} />
                  <Row k="Line / Distance" v={`${data.overlays.nearest_metro_line} — ${data.overlays.distance_to_metro}`} />
                </>
              )}
              <Row k="AB 2097 Zero Parking" v={pill(data.overlays?.ab2097_zero_parking, 'Exempt', 'Required')} />
              <div style={{ margin: '14px 0 8px', borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: 12, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#64748b' }}>Federal Overlays</div>
              <Row k="Opportunity Zone" v={pill(data.overlays?.opportunity_zone, 'Yes — OZ', 'No')} />
              <Row k="Low Income Area" v={pill(data.overlays?.low_income_area, 'Yes — LMI', 'No')} />
            </Card>
          </div>

          {/* Row 2: Valuation + Development */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>

            <Card label="Assessed Value" accent="#7c6fff">
              <div style={{ fontSize: '2.4rem', fontWeight: 700, color: '#7c6fff', lineHeight: 1, margin: '8px 0 4px' }}>
                ${Math.round((data.valuation?.total_assessed || 0) / 1000)}K
              </div>
              <div style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>Total assessed (Prop 13 base)</div>
              ...
              <Row k="Tax Status" v={data.valuation?.tax_status || '—'} />
            ...
            <Card label="Development Potential">
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 18 }}>
                {[
                  { num: data.development_potential?.base_units_by_right, lbl: 'By Right', color: '#e2e8f0' },
                  ...
                  <div key={lbl} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '12px 14px', textAlign: 'center' }}>
                    <div style={{ fontSize: '1.8rem', fontWeight: 700, color }}>{num ?? '—'}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            ...
            <Card label="SB 9 — Urban Lot Split / Duplex" accent="#4caf7d">
              <div style={{ marginBottom: 14 }}>
                {pill(data.sb9?.eligible, 'SB 9 Eligible', 'Not Applicable')}
              </div>
              {data.sb9?.eligible ? (
                <>
                  <Row k="Duplex Units" v={<span style={{ color: '#4caf7d', fontWeight: 700 }}>{data.sb9.duplex_units} units by-right</span>} />
                  <Row k="Lot Split" v={pill(data.sb9?.lot_split_eligible, 'Allowed', 'Not Feasible')} />
                  {data.sb9?.lot_split_eligible && (
                    <>
                      <Row k="Max Units (w/ split)" v={<span style={{ color: '#f5a623', fontWeight: 700 }}>{data.sb9.max_units_with_split} units</span>} />
                      {data.sb9?.resulting_lots?.map((l: string) => (
                        <Row key={l} k="" v={<span style={{ color: '#94a3b8', fontSize: 13 }}>{l}</span>} />
                      ))}
                    </>
                  )}
                  <Row k="Approval" v={data.sb9?.approval_path} />
                  {data.sb9?.notes && <div style={{ marginTop: 12, fontSize: 13, color: '#64748b', lineHeight: 1.6 }}>{data.sb9.notes}</div>}
                  {data.sb9?.owner_occupancy && <div style={{ marginTop: 8, fontSize: 12, color: '#e8c54a', background: 'rgba(232,197,74,0.08)', padding: '8px 12px', borderRadius: 6 }}>&#x26A0; {data.sb9.owner_occupancy}</div>}
                </>
              ) : (
                <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6 }}>{data.sb9?.reason}</div>
              )}
            </Card>

            <Card label="ADU — Accessory Dwelling Unit" accent="#4caf7d">
              <div style={{ marginBottom: 14 }}>
                {pill(data.adu?.eligible, 'ADU Eligible', 'Not Applicable')}
              </div>
              {data.adu?.eligible ? (
                <>
                  <Row k="ADUs Allowed" v={<span style={{ color: '#4caf7d', fontWeight: 700 }}>{data.adu.adu_count_allowed} unit{data.adu.adu_count_allowed > 1 ? 's' : ''}</span>} />
                  <Row k="Max ADU Size" v={`${data.adu.max_adu_sqft?.toLocaleString()} sq ft`} />
                  <Row k="Detached ADU" v={pill(data.adu?.detached_adu, 'Allowed', 'Check setbacks')} />
                  <Row k="JADU" v={pill(data.adu?.jadu_eligible, `Yes — up to ${data.adu.jadu_max_sqft} sqft`, 'No')} />
                  <Row k="Parking Required" v={pill(!data.adu?.parking_required, 'None Required', 'Required')} />
                  <Row k="Permit Type" v={data.adu?.permit_type} />
                  {data.adu?.rental_income_estimate && (
                    <div style={{ marginTop: 14, background: 'rgba(76,175,125,0.08)', border: '1px solid rgba(76,175,125,0.2)', borderRadius: 8, padding: '12px 14px' }}>
                      <div style={{ fontSize: 11, color: '#4caf7d', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Est. Rental Income</div>
                      <div style={{ fontSize: '1.2rem', fontWeight: 700, color: '#4caf7d' }}>{data.adu.rental_income_estimate}</div>
                    </div>
                  )}
                </>
              ) : (
                <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6 }}>{data.adu?.reason}</div>
              )}
            </Card>
          </div>

          {/* Row 4: Grants (full width) */}
          <div style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 22, marginBottom: 16 }}>
            <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#64748b', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
              {[
                ...
                <div key={label} style={{ textAlign: 'center', padding: '12px 28px', background: bg, border: `1px solid ${border}`, borderRadius: 10 }}>
            ...
            {data.grants?.qualified?.length > 0 && <>
              ...
              {data.grants.qualified.map((g: any) => (
                <div key={g.name} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '16px 18px', marginBottom: 10 }}>
                  ...
                  <div style={{ color: '#64748b', fontSize: 13, marginBottom: 8 }}>{g.agency} ·{' '}
                    <span style={{ background: g.level==='federal' ? 'rgba(124,111,255,0.15)' : g.level==='state' ? 'rgba(76,175,125,0.15)' : 'rgba(232,197,74,0.15)', color: g.level==='federal' ? '#7c6fff' : g.level==='state' ? '#4caf7d' : '#e8c54a', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{g.level}</span>
                  </div>
                  ...
                  <a href={g.url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 8, color: '#7c6fff', fontSize: 13, textDecoration: 'none' }}>Learn more &rarr;</a>
                </div>
              ))}
            </>}
            ...
            {data.grants?.potential?.length > 0 && <>
              ...
              {data.grants.potential.map((g: any) => (
                <div key={g.name} style={{ border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '14px 18px', marginBottom: 8, opacity: 0.8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{g.name}</div>
                    <div style={{ color: '#f5a623', fontSize: 13 }}>{g.max_award}</div>
                  </div>
                  <div style={{ color: '#64748b', fontSize: 13, marginTop: 4 }}>{g.agency} — {g.description}</div>
                  <a href={g.url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 6, color: '#7c6fff', fontSize: 13, textDecoration: 'none' }}>Learn more &rarr;</a>
                </div>
              ))}
            </>}
          </div>
          ...
          {/* Live Grants.gov */}
          {data.grants?.live?.length > 0 && (
            <div style={{ background: '#111827', border: '1px solid rgba(124,111,255,0.2)', borderRadius: 12, padding: 22, marginBottom: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7c6fff', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#7c6fff', display: 'inline-block' }} />
                Live Grants.gov Opportunities ({data.grants.live.length})
              </div>
              {data.grants.live.map((g: any, i: number) => (
                <div key={i} style={{ border: '1px solid rgba(124,111,255,0.15)', borderRadius: 10, padding: '14px 18px', marginBottom: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 4 }}>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{g.title}</div>
                    {g.award && <div style={{ color: '#f5a623', fontSize: 13, whiteSpace: 'nowrap' }}>{g.award}</div>}
                  </div>
                  <div style={{ color: '#64748b', fontSize: 13, marginBottom: 6 }}>
                    {g.agency}&nbsp;&nbsp;
                    <span style={{ background: g.status === 'posted' ? 'rgba(76,175,125,0.15)' : 'rgba(232,197,74,0.15)', color: g.status === 'posted' ? '#4caf7d' : '#e8c54a', padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{g.status}</span>
                    {g.close_date && <span style={{ marginLeft: 10, color: '#475569' }}>Closes: {g.close_date}</span>}
                  </div>
                  {g.ref && <div style={{ fontSize: 12, color: '#475569', marginBottom: 4 }}>Ref: {g.ref}</div>}
                  <a href={g.url} target="_blank" rel="noopener noreferrer" style={{ color: '#7c6fff', fontSize: 13, textDecoration: 'none' }}>Apply on Grants.gov &rarr;</a>
                </div>
              ))}
            </div>
          )}
          ...
          {data.next_steps?.length > 0 && (
            <div style={{ background: '#111827', border: '1px solid rgba(245,166,35,0.2)', borderRadius: 12, padding: 22, marginBottom: 16 }}>
              <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#f5a623', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#f5a623', display: 'inline-block' }} />
                Recommended Next Steps
              </div>
              {data.next_steps.map((s: any) => (
                <div key={s.priority} style={{ display: 'flex', gap: 16, padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ minWidth: 32, height: 32, borderRadius: '50%', background: 'rgba(245,166,35,0.15)', border: '1px solid rgba(245,166,35,0.3)', color: '#f5a623', fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{s.priority}</div>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 4 }}>{s.action}</div>
                    <div style={{ color: '#64748b', fontSize: 13, lineHeight: 1.55, marginBottom: 6 }}>{s.reason}</div>
                    <a href={s.url} target="_blank" rel="noopener noreferrer" style={{ color: '#7c6fff', fontSize: 13, textDecoration: 'none' }}>Learn more &rarr;</a>
                  </div>
                </div>
              ))}
            </div>
